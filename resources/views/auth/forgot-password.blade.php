...
@section('content')
<section class="bg-[#0006] text-white rounded-lg w-4/12 gap-2 p-4 flex flex-col items-center justify-center">
    ...
    <div class="card w-full max-w-sm">
        ...
        <form method="POST" action="{{ route('password.email') }}" class="card-body">
            ...
            <input type="text" class="input bg-[#0009] indent-2 outline-0" name="email" placeholder="Email" value="{{ old('email') }}" />
            @error('email')
            <small class="badge badge-error w-full -mt-1 text-xs py-4">{{ $message }}</small>
            @enderror

            <button class="btn btn-outline hover:bg-[#fff6] hover:text-white mt-4 border">Email Password Reset Link</button>
